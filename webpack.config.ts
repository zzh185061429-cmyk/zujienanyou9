...
<<<<<<< HEAD
      alias: {
        ...
        react$: path.join(path.dirname(require.resolve('react/package.json')), 'cjs', 'react.production.js'),
        'react-dom$': path.join(path.dirname(require.resolve('react-dom/package.json')), 'cjs', 'react-dom.production.js'),
        // react-dom/client 可能被 webpack 解析为 react-dom/client.js，
        // $ 精确匹配会遗漏 .js 后缀，因此用前缀匹配
        'react-dom/client': path.join(path.dirname(require.resolve('react-dom/package.json')), 'cjs', 'react-dom-client.production.js'),
...
=======
      alias: {},
>>>>>>> 4eaa852d0b2d1f8c70fd2db2aa5d5283d32e00f8
    },
...
<<<<<<< HEAD
      ...
      // motion 核心包不可 external 到 CDN，否则 CDN 的 motion / framer-motion 自带第二份 React 实例
      // 导致 motion/react（已打包进 bundle）和 motion（CDN）引用不同 React
      // motion/react 内部 import from 'framer-motion'，须一并拦截
      // motion v12+ 拆分了 motion-dom / motion-utils 子包，同样不可 external
      if (
        request === 'motion' ||
        request.startsWith('motion/') ||
        request === 'motion-dom' ||
        request.startsWith('motion-dom/') ||
        request === 'motion-utils' ||
        request.startsWith('motion-utils/') ||
        request === 'framer-motion' ||
        request.startsWith('framer-motion/')
      ) {
        return callback();
      }
      // clsx / tailwind-merge 不可 external 到 CDN：
      // 1. 前端界面 iframe 可能因 CSP 限制无法加载 CDN 资源
      // 2. 这两个包体积小（<5KB gzip），打包进 bundle 代价极低
...
<<<<<<< HEAD
      ...
      if (
        ...
        fs.existsSync(path.join(import.meta.dirname, '示例', request))
      ) {
